<head>
    <meta name="viewport" content="width=device-width initial-scale=1.0" />
    <title>Camila Guzman</title>

    <!--stylesheet-->
    <link rel="stylesheet" href="styles/main.css" />

    <!--icon kit-->
    <script src="https://kit.fontawesome.com/bf0ea5bc59.js" crossorigin="anonymous"></script>

    <!--jquery-->
    <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.5.1/jquery.min.js"></script>

    <!-- bootstrap-->

    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-1BmE4kWBq78iYhFldvKuhfTAU6auU8tT94WrHftjDbrCEXSU1oBoqyl2QvZ6jIW3" crossorigin="anonymous">

    <!-- functions-->
    <script src="script/functions.js"></script>
</head>

<body>
    <div class="container-fluid">
        <div class="row">
    <div class="nav-div d-lg-none sticky-top">
        <header>
            <div class="logo"></div>
            <!--title: name-->
            <a href="index.php">
                <h2 class="name">Camila Guzman</h2>
            </a>

            <button id="toggle-nav" class="d-flex">
                <i class="fas fa-bars"></i>
            </button>
        </header>
        <div class="nav-content">
            <p>
            Curious developer and designer who loves learning new techniques and tools to improve user experiences.
            </p>

            <!--social nav-->
            <nav class="social-nav">
                <a href="https://github.com/camila-guzman">@camila-guzman on GitHub</a><br />
            </nav>

            <nav class="nav-bar">
                <a href="index.php#about"> <span>About</span></a>
                <a href="index.php#portfolio"> <span>Portfolio</span></a>
                <a href="index.php#contact"><span>Contact</span></a>
            </nav>
        </div>
    </div>
    <div class="col-lg-4 d-none d-lg-block">
        <div class="sidebar container-fluid  sticky-top">
            <div class="mx-auto p-2">
                <div class="logo"></div>

                <!--title: name-->
                <h2 class="name">Camila Guzman</h2>

                <p>
                    Curious developer and designer who loves learning new techniques and tools to improve user experiences.
                </p>

                <!--social nav-->
                <div class="mt-2">
                    <nav class="social-nav">
                        <li><a href="https://github.com/camila-guzman"><span><i style="margin-right: 10px" class="fab fa-github-alt"></i></span>@camila-guzman on GitHub</a></li>
                        <li><a href="https://www.linkedin.com/in/camila-guzman11/"><span><i style="margin-right: 10px" class="fab fa-linkedin-in"></i></span>Add me on LinkedIn</a></li>
                        <li><a href="mailto:camilamguzman11@gmail.com"><span><i style="margin-right: 10px" class="fas fa-envelope"></i></span>camilamguzman11@gmail.com</a></li>
                    </nav>
                </div>
                <!--page nav-->
                <div class="mt-2">
                    <nav class="page-nav">
                        <a href="#about"> <span>About</span></a>
                        <a href="#portfolio"> <span>Portfolio</span></a>
                        <a href="#contact"><span>Contact</span></a>
                    </nav>
                </div>
        </div>
    </div>
</div>